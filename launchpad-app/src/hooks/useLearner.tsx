/* ============================================================================
   LAUNCHPAD — learner context (ORBIT's)
   ----------------------------------------------------------------------------
...
import type { ClaimPlan } from '@/engine/claims'
import type { Dag } from '@/engine/graph'
import { dueAtoms, masteryMap } from '@/engine/scheduler'
import { dayKey, newLearnerState, type LearnerState } from '@/engine/state'
...
interface LearnerContextValue {
  ...
  /**
   * Apply a planned change to the LAUNCHPAD claims record (engine/claims.ts).
   * A refused plan returns its reason; a destructive one asks first, in the
   * words the plan supplies, and returns 'cancelled' if the answer is no.
   */
  claim: (plan: ClaimPlan) => { result: 'done' | 'cancelled' | 'blocked'; message?: string }
...
export function LearnerProvider({ children }: { children: ReactNode }) {
  ...
  const claim = useCallback<LearnerContextValue['claim']>(
    (plan) => {
      if (plan.kind === 'blocked') return { result: 'blocked', message: plan.message }
      if (plan.kind === 'confirm' && !window.confirm(plan.lines.join('\n'))) return { result: 'cancelled' }
      // Stamp the day's readiness so the trend chart moves when a gate passes,
      // not only when a card is graded. A stamp alone is not activity: the
      // streak still counts study, not ticking.
      const next = plan.state
      const key = dayKey(new Date())
      const prev = next.days[key] ?? { date: key, reviews: 0, correct: 0, newItems: 0, minutes: 0, readiness: 0 }
      const stamped: LearnerState = {
        ...next,
        days: { ...next.days, [key]: { ...prev, readiness: dag.readiness(masteryMap(next, MODULES)) } },
      }
      stateRef.current = stamped
      setStateRaw(stamped)
      // A claim is a record of work done; it is written at once.
      saveStateNow(stamped)
      setRevision((r) => r + 1)
      return { result: 'done' }
    },
    [dag],
