...
export function Settings() {
  ...
  const claimed = Object.values(state.claims).filter((c) => c.delta || c.artifact || c.gate).length
  ...
  const onRestore = async (file: File) => {
    ...
    if (result.claimsOnly) {
      // A backup from the old LAUNCHPAD realm: it carries the claims, the hard
      // gate and the plan. Those are replaced; recall history is left alone.
      if (!window.confirm('Replace all LAUNCHPAD progress with this backup?')) return
      setState((s) => ({
        ...s,
        claims: result.state.claims,
        flagship: result.state.flagship,
        setup: result.state.setup,
        legacyImportedAt: s.legacyImportedAt ?? new Date().toISOString(),
      }))
      setNotice({
        tone: 'ok',
        text: `LAUNCHPAD progress restored: ${Object.keys(result.state.claims).length} modules with claims, from a backup of the old realm.`,
      })
      return
    }
    ...
    setNotice({
      tone: 'ok',
      text: `Restored ${Object.keys(result.state.claims).length} modules' claims, ${Object.keys(result.state.items).length} words and ${result.state.attempts.length} logged attempts.`,
    })
  }
        ...
        <div className="stack">
          ...
          <Card index={0}>
            <CardHead
              ...
              right={<Chip tone={items + claimed > 0 ? 'warn' : 'default'}>{claimed} modules · {items} words</Chip>}
              divided
            />
            <div className="setting">
              <div className="grow">
                ...
                <p className="setting__help">
                  A single JSON file containing your claims — every delta, checkpoint, artifact and
                  gate — the hard gate, your plan, your recall history and settings. Keep a copy
                  somewhere that is not this device — that file is the durable version of your
                  progress.
                </p>
              </div>
            ...
            <div className="setting">
              <div className="grow">
                ...
                <p className="setting__help">
                  ...
                  partially applied. A backup exported by the old LAUNCHPAD realm in APEX restores too:
                  it replaces your claims, the hard gate and the plan, and leaves recall alone.
                </p>
              </div>
              <div className="setting__control">
                <input
        ...
        <div className="stack">
          ...
          <Card index={1}>
            ...
            <div className="sect" style={{ fontSize: 12, color: 'var(--ink-3)', lineHeight: 2 }}>
              <div>{stats.modules} modules on 9 layers</div>
              <div>{stats.lessons} lessons, from the curriculum document</div>
              <div>{stats.cards.toLocaleString('en-US')} words to recall</div>
              <div>{Math.round(stats.hours).toLocaleString('en-US')} module hours, plus the parallel tracks</div>
          ...
          <Card index={2}>
            ...
            <div className="sect" style={{ fontSize: 12, color: 'var(--ink-3)', lineHeight: 1.75 }}>
              ...
              only outbound requests this app makes are, if you open the playground, for the Python
              and SQLite runtimes — both from a public CDN, both cached after the first load.
              JavaScript runs in the browser&rsquo;s own engine and needs nothing downloaded.
            </div>
          </Card>
          ...
          <Card index={3}>
            ...
            <div className="sect" style={{ fontSize: 12, color: 'var(--ink-3)', lineHeight: 1.75 }}>
              Words are scheduled by FSRS-6, a fitted power-law memory model with per-item stability
              and difficulty, and the Progress page shows all of it rather than hiding it behind a
              score. What opens a module is not the scheduler: it is the gate — a referee&rsquo;s yes on
              every module it depends on — recorded by you on the module page.
            </div>
          </Card>
        </div>
      </div>
