/* ============================================================================
   LAUNCHPAD — the scheduler (ORBIT's)
   ----------------------------------------------------------------------------
...
import { AI_LAYERS } from '@/curriculum/generated/launchpad-data'

import { trackForLayer } from '@/curriculum/tracks'
import { AI_CURRICULUM, AI_CURRICULUM_API } from '@/curriculum/generated/launchpad-data'
import { claimProgress, claimsMasteryMap, isPassed, progressOf } from './claims'
...
/**
 * The mastery map the rest of the engine runs on.
 *
 * In LAUNCHPAD a module's progress is its claims record — delta, checkpoints,
 * artifact, and a referee's yes at the gate — not its recall score: the
 * curriculum's first rule is that you advance by shipping a working thing, not
 * by reading. So this is the claims map (engine/claims.ts), in which a module
 * reaches the prerequisite threshold exactly when its gate is passed, and the
 * hard gate rides along under its own id. Recall still has its own number,
 * `moduleMastery` above, for the review pages that are about memory.
 */
export function masteryMap(
  ...
  const all = claimsMasteryMap(state)
  const out = new Map<string, number>()
  for (const m of modules) out.set(m.id, all.get(m.id) ?? 0)
  for (const [k, v] of all) if (!out.has(k) && k.startsWith('__')) out.set(k, v)
  return out
}
...
export function rankFrontier(
  ...
  const out = frontier.map((m) => {
    ...
    // A module a date or an event starts is open, but not next: it waits for
    // its moment rather than for readiness, so it ranks after everything else.
    const scheduled = m.lp?.trigger ? 1 : 0
    const score = 0.4 * fit + 0.25 * urgency + 0.2 * explore + 0.15 * leverage - scheduled
...
export function buildSession(
  ...
  if (room > 0) {
    ...
    // A passed module leaves the frontier, but its words are still worth
    // learning, so its unseen cards follow the frontier's. A session asked for
    // one module by name draws from it even while it is locked: reading ahead
    // is allowed, and so is learning its vocabulary.
    const rankedIds = new Set(ranked.map((c) => c.module.id))
    const extra = modules
      .filter((m) => !rankedIds.has(m.id))
      .filter((m) => !!opts.moduleId || (mastery.get(m.id) ?? 0) >= MASTERY_THRESHOLD)
      .map((m) => ({ module: m }))

    for (const cand of [...ranked, ...extra]) {
      if (room <= 0) break
...
export function dailyPlan(
  ...
  /*
   * LAUNCHPAD's "today's focus": when every module up to Layer 3 has passed
   * and the hard gate is still shut, the next thing is the gate itself, and
   * nothing on the ladder moves it. Otherwise the modules open right now, in
   * ladder order, leaving out the ones a date or an event starts — those are
   * open, but each one says inside when to begin.
   */
  ...
  const finished = AI_CURRICULUM.every((m) => isPassed(state, m.id))
  if (l03 && !fg.open && !finished) {
    tasks.push({
      id: 'hard-gate',
      title: 'Next: the hard gate',
      context: `${fg.done}/${fg.total} conditions · the work is off the ladder`,
      minutes: 60,
      href: '#/learning?show=gate',
      done: done('hard-gate'),
      tone: 'habit',
    })
  }

  for (const m of AI_CURRICULUM_API.available(p).filter((x) => !x.trigger).slice(0, 4)) {
    const started = claimProgress(state, m.id) > 0
    tasks.push({
      id: `study-${m.id}`,
      title: started ? `Continue ${m.title}` : `Start ${m.title}`,
      context: `${m.id} · ${m.hours}h · ${m.kind === 'EVIDENCE' ? 'Evidence' : 'Lab'}`,
      minutes: 30,
      href: `#/module/${m.id}`,
...
export function trackLabel(track: string): string {
  return AI_LAYERS.find((L) => trackForLayer(L.id) === track)?.name ?? track
}
