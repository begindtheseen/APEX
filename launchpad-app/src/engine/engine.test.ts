...
import { atomsOf, buildSession, dueAtoms, interleave, optimalGapDays, rankFrontier, retentionForDeadline } from './scheduler'
...
describe('applyAttempt', () => {
  ...
  it('stamps readiness onto the day when a roll-up is supplied', () => {
    // The roll-up itself is the caller's (in LAUNCHPAD, the share of hours whose
    // gate has passed); what applyAttempt owns is putting its answer on the day.
    const s = applyAttempt(newLearnerState(now), { itemId: atom.id, grade: 4 }, root, now, () => 0.42).state
    expect(s.days[dayKey(now)]!.readiness).toBeCloseTo(0.42, 6)
  })
