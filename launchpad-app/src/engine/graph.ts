/* ============================================================================
   LAUNCHPAD — prerequisite graph (ORBIT's)
   ----------------------------------------------------------------------------
...
import { HARD_GATE_ID, isHardGated } from './claims'
...
export class Dag {
   ...
   *
   * In LAUNCHPAD a prerequisite reaches the threshold only when its gate is
   * passed, and Layers 4–6 have one more lock that is not a module: the hard
   * gate, reported as `HARD_GATE_ID` whenever the mastery map says it is shut.
   */
  blockers(id: string, mastery: ReadonlyMap<string, number>): string[] {
    const out = this.prereqs(id).filter((p) => (mastery.get(p) ?? 0) < PREREQ_THRESHOLD)
    const layer = this.modules.get(id)?.lp?.layer
    if (layer != null && isHardGated(layer) && mastery.has(HARD_GATE_ID) && mastery.get(HARD_GATE_ID)! < 1) {
      out.push(HARD_GATE_ID)
    }
    return out
  }
  ...
  /**
   * The headline number. LAUNCHPAD has always stated it one way — the share of
   * module hours whose gate a referee has passed — and the ring keeps that
   * meaning rather than ORBIT's weighted blend: a module counts, at its hours,
   * once it is done, and not before. Partial work shows on the module itself.
   */
  readiness(raw: ReadonlyMap<string, number>, filter?: (m: Module) => boolean): number {
    ...
    for (const id of this.order) {
      ...
      den += m.hours
      if ((raw.get(id) ?? 0) >= MASTERY_THRESHOLD) num += m.hours
    }
