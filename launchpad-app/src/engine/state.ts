/* ============================================================================
   LAUNCHPAD — learner state (ORBIT's, plus the LAUNCHPAD claims record)
   ----------------------------------------------------------------------------
   ...
   ========================================================================== */
import type { LpSetup } from '@/curriculum/generated/launchpad-data'
import { coerceClaims, coerceFlagship, coerceSetup, type Claim } from './claims'
...
export interface LearnerState {
  ...
  /**
   * The LAUNCHPAD record: per module, the delta written, each checkpoint
   * ticked, the artifact built and the gate passed. This — not recall — is
   * what opens the next module. See engine/claims.ts for the rules.
   */
  claims: Record<string, Claim>
  /** The hard gate's four conditions, ticked. */
  flagship: Record<string, true>
  /** The Plan's inputs: runway, weekly hours, the flagship's name. */
  setup: LpSetup
  /**
   * ISO time the old realm's record (localStorage `apex_launchpad_v1`) was
   * read into this state, so it is carried across once and never again.
   */
...
export function migrateState(raw: unknown, now: Date = new Date()): LearnerState {
  ...
  const out: LearnerState = {
    ...
    claims: coerceClaims(r.claims),
    flagship: coerceFlagship(r.flagship),
    setup: coerceSetup(r.setup),
    parked: coerceParked(r.parked),
    version: STATE_VERSION,
  }

  const legacyImportedAt = str(r.legacyImportedAt)
  if (legacyImportedAt) out.legacyImportedAt = legacyImportedAt
