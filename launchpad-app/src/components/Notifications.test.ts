...
describe('notesFor', () => {
  ...
  const resume = (progress: number) => ({
    ...
    path: '/module/M3?lesson=m3-the-module',
    label: 'The Runtime, Unframed',
    detail: 'Lesson 1 of 2 · 60% through',
    progress,
  ...
  it('links back to the exact place, not the top of the module', () => {
    ...
    expect(note?.href).toBe('/module/M3?lesson=m3-the-module')
    expect(note?.detail).toBe('Lesson 1 of 2 · 60% through')
  })
