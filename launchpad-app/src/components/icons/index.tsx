...
/** A flag on a pole — a checkpoint, a place you can stop and come back to. */
export const IconFlag = (p: IconProps) => (
  <Svg {...p}>
    <path d="M5.6 21V3.8" />
    <path d="M5.6 4.4h11.2l-2.4 4 2.4 4H5.6" />
  </Svg>
)

/** A circle with a bar and a dot — something commonly gotten wrong. */
...
export function Wordmark({ height = 16, ...rest }: SVGProps<SVGSVGElement> & { height?: number }) {
  return (
    <svg
      ...
      {...rest}
    >
      {/* L A U N C H P A D — set in the display face, wide-tracked like the
          realm's own wordmark, with PAD in the accent it always carried. */}
      <text
        x="0"
        y="23"
      ...
      >
        LAUNCH<tspan fill="var(--accent, #4d9bff)">PAD</tspan>
      </text>
    </svg>
  )
}
...
/**
 * The standalone mark — the realm's hexagon, drawn as two strokes: an outline,
 * and a rising trail leaving its upper vertex, which is the whole idea of a
 * launch pad in one line.
 */
export function Logomark({ size = 48, ...rest }: SVGProps<SVGSVGElement> & { size?: number }) {
  return (
    <svg viewBox="0 0 120 56" width={size} fill="none" role="img" aria-label="LAUNCHPAD" {...rest}>
      <path
        d="M40 8 57.3 18v20L40 48 22.7 38V18Z"
        stroke="currentColor"
        strokeWidth="3.2"
        strokeLinejoin="round"
      />
      <path d="M57.3 18C76 14 94 10 116 8" stroke="currentColor" strokeWidth="3.2" strokeLinecap="round" />
    </svg>
  )
}
