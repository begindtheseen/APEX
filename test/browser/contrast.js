...
// Measures every text/background pair LAUNCHPAD renders, page by page, and
// reports the ones below the WCAG threshold.
...
await LP.reset(p);
await LP.open(p,'/');
const routes=['/','/learning','/plan','/tracks','/contract','/progress','/review','/guide','/resources','/playground','/settings'];
for (const id of ['M0','M1','M5','M12','M18']) routes.push('/module/'+id+'?step=learn','/module/'+id+'?step=build');
let all=[];
for (const r of routes){
  await LP.go(p,r);
  all=all.concat(await p.evaluate(measure));
}
