...
(async () => {
  ...
  for (const [w,h,label] of [[390,844,'390'],[1280,900,'1280']]) {
    console.log('\n════ '+label+'px ════');
    for (const [sel,name,appId] of [['.lsObs','OBSIDIAN','obsidianApp'],['.lsRed','REDLINE','obsidianApp'],['.lsLp','LAUNCHPAD','launchpadApp']]) {
      ...
      // LAUNCHPAD's top-left button is a back step everywhere but Home, and
      // only leaves the realm from Home. The other realms exit directly.
      ok('leaves the realm only from home', await (async()=>{
        for (let i=0;i<4;i++){
