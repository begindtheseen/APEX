...
{ id:'M1', layer:0, hours:58, dependsOn:['M0'], kind:'LAB',
  note:'This is where code starts, and it starts from nothing: what a program is, what the terminal is, what a variable is. Fifty-eight hours is the longest module in this layer on purpose — it is the one that decides whether the rest of the program is readable to you. Nothing before this assumed you could write anything, and nothing here assumes it either.',
  title:'First Code',
...
{ id:'M2', layer:0, hours:30, dependsOn:['M1'], kind:'EVIDENCE',
  note:'The spec checklist asks what this app must be able to grow into, and the instruction is to pick a different app now if it cannot. Read that seriously, because those lines pay off between M11 and M26 — months eight to eighteen — and you are choosing in month two. If one fails anyway that late: a paid tier, a second role, a third-party account and a CI pipeline all bolt on to a running app, 10 to 15 hours for whichever you are repairing. A user-uploaded corpus and a streaming chat surface do not, because M11, M18 and M22 measure and rebuild what your app already is; if your flagship has neither, those three lose their substrate. A second small app is a partial fallback — M20, M21 and M26 will run on one, M11, M18 and M22 will not. The cost is Track 3: every module riding the second app is one whose artifact no longer builds on the one before it. Those hours are not in the 1,578. Log it in INCIDENTS.md.',
  title:'Flagship v1',
...
{ id:'M11', layer:3, hours:36, dependsOn:['M5','M8','M10'], kind:'EVIDENCE',
  ...
  gate:{ referee:'Your reviewer, scoring against your twenty hand-labeled documents. A number you cannot fudge.',
         pass:'State what percentage of tables your parser destroys, with evidence. Show a citation that highlights the exact span in the correct page of the correct source. Say which extraction path each document type goes through, with the vision arm’s table score and its provenance result on the record — kept or cut, and why.',
...
{ id:'M16', layer:4, hours:28, dependsOn:['M14','M15'], kind:'EVIDENCE',
  ...
  gate:{ referee:'A second agent instance with a sealed brief your reviewer wrote weeks earlier, in M15. 0-3 defects per diff, count blinded, at least one clean diff.',
...
{ id:'M25', layer:6, hours:35, dependsOn:['M24','M12'], kind:'EVIDENCE',
  ...
  gate:{ referee:'A timer, plus a Python-fluent OSS maintainer reviewing a real PR — route one Track 2 contribution to a Python repo.',
...
var AI_CURRICULUM_API = {
  ...
  plan: function (setup) {
    ...
    var s = setup || {};
    // A negative is not a small number here, it is a refused one: it would make
    // every division below produce a negative duration and a confident verdict.
    var wkEntered = Math.max(0, Number(s.weeklyHours) || 0);
    ...
    var runway = Math.max(0, Number(s.runwayMonths) || 0);
    ...
    // And after it. T1 and T9 switch on at the application date, and T2 reaches its
    // full eight hours a month, so track load goes from about 2.3 h/week to about 8
    // and module time drops from roughly sixteen hours a week to ten. Nothing below
    // divides by this — the application-date projection is a pre-apply question, and
    // the full/Spine durations already carry track hours inside their totals. It is
    // here because the drop is real, the document states it, and a number the prose
    // asserts should exist in the data rather than only in a paragraph.
    var POST_APPLY_TRACK_RATE = { T1:3.5, T2:1.85, T3:0, T4:0.75, T5:0.25, T6:0, T7:0.4, T8:0.46, T9:0.5, T10:0.25 };
    var postTrack = Object.keys(POST_APPLY_TRACK_RATE).reduce(function (a, k) { return a + POST_APPLY_TRACK_RATE[k] }, 0);
    ...
    if (!runway || !wk) {
      rec = 'unset';
      var bad = [];
      if (s.runwayMonths !== undefined && s.runwayMonths !== '' && !runway) bad.push('a runway of ' + s.runwayMonths + ' months');
      if (s.weeklyHours !== undefined && s.weeklyHours !== '' && !wkEntered) bad.push(s.weeklyHours + ' hours a week');
      why = bad.length
        ? 'That is not a plan: ' + bad.join(' and ') + '. Both have to be above zero, because every number on '
          + 'this page is one divided by the other. Put in the smallest honest figures you can live with.'
        : 'Enter your runway and the hours you can actually commit. Everything downstream derives from these two numbers.';
    // A tenth of an hour a week short is not a different plan. Rounding up
    // covers it, and shouting "TIGHT — RAISE HOURS" at a six-minute gap makes
    // the same warning meaningless when the gap is real.
    } else if (needFull && needFull <= wk + 0.25) {
      rec = 'full';
    ...
    return {
      ...
      // The same two numbers on the far side of the application date, where T1 and
      // T9 switch on and T2 reaches full cadence. Module progress per calendar week
      // falls by about a third and stays there for the rest of the program. Returned
      // so a consumer can warn instead of letting the reader carry the pre-apply rate
      // forward and read the gap as falling behind.
      postApplyTrackHoursPerWeek: postTrack,
      postApplyModuleHoursPerWeek: (wk > postTrack ? wk - postTrack : 0),
...
M11:[
 ...
 ['Vision model','A model you hand the picture of a page to instead of the text, and it reads it. On a scanned or multi-column document it often keeps a table intact where OCR turns it into word soup — but it gives you back what it read, not where on the page it read it, so character offsets are the thing it is worst at. M11 makes you measure both and say which one you kept.'],
...
var AI_CHECKPOINTS = {
  ...
  M6:['Token counts compared against your assumptions across five text types',
      ...
      'Every stop_reason this module can reach produced on purpose, with the refusal taken from a recorded fixture, and the cost column live in the M2 log',
  ...
  M11:['Twenty documents hand-labeled and about twenty-five queries hand-written against them, split into dev and test before any pipeline code is written, with the number in each half recorded',
      ...
      'The same twenty documents through a vision model as a second extraction arm, scored on the same table metric and priced per document, with what happened to character offsets written down'],
