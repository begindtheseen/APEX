...
M22:{concepts:['React from zero: components, props, state — enough to own one page, not a framework tour','The React rendering model: what actually causes a re-render','Effects and their four failure modes','The server/client boundary and the current caching direction (opt-in, not opt-out)','Forms, mutations, optimistic UI with shared Zod schemas','A custom transport over your own SSE frames','Cancellation, abort propagation, resumable streams','Latency choreography for 10-second-plus operations','Conversation scroll behavior and accessible streaming','Designing for output that is sometimes wrong'],
 mistakes:['useState as a variable store kept in sync with useEffect.','Adding and removing dependencies (the list of values an effect re-runs on) until the lint rule goes quiet.','"use client" at the root layout, converting the whole tree to client components.','Assuming one read() chunk equals one complete SSE event. Corrupts output only under load.','setState on every token at 60/sec, re-rendering the whole markdown tree, then blaming React.','Conflating client disconnect with user cancellation — or a stop button that stops the UI while the server keeps generating and charging.','aria-live="polite" on the streaming container, making screen readers re-read the entire growing message.','An approval button that appears after the tool already ran. That is theatre, not a gate.']},
...
M2:[
 ...
 ['Log','A file your server adds one line to on every request: what came in, what went out, when (the timestamp), and what it cost.'],
 ['Streaming, accounts, database','Three things this version deliberately lacks. Streaming shows the reply word by word as it arrives (M7). Accounts let a user log in (added with Supabase, a hosted database service, in M5; connecting other services is M26). A database keeps data between visits (M5).'],
...
M13:[
 ...
 ['Mock interviewer','A person playing the interviewer for practice, asking follow-ups you did not prepare for. Track 5 (Tracks tab) has you find one before this module.'],
...
M17:[
 ...
 ['Nullable','A column allowed to hold no value (null). A new column is added nullable so existing rows stay valid while it is being filled.'],
 ['Dual write','Writing the same data to the old column and the new one during a change, so both stay correct until the old one is dropped.']],
M18:[
 ...
 ['dist','The distance between two vectors: the smaller it is, the closer in meaning the two texts are. The figure labels it dist; a nearest-neighbour search is a search for the smallest one.']],
M19:[
...
M21:[
 ...
 ['Browser code','Any file the browser downloads: the frontend, or client side, of the app, which from M22 is built from components (single pieces of the page). Anything it imports ships to the browser too, secrets included.'],
...
M23:[
 ...
 ['Nullable','A column allowed to hold no value (null). The expand step adds the new column nullable so old rows stay valid until it is filled.'],
...
M27:[
 ...
 ['Funnel table','A table you keep of the job search by stage: applications sent, replies, screens, interview loops (an interview loop is the full set of interview rounds a company runs for one candidate), offers, one column per channel. Called a funnel because each stage is smaller than the one before; it shows which channel is working.'],
...
M29:[
 ['Layer','The program\'s grouping of modules into stages, numbered 0 to 8. Layer 5 is the AI modules M18 to M21. This module waits for M18 and M20 in particular, because that is where its numbers come from.'],
...
M30:[
 ['Forward-loaded','Done early, ahead of the date, instead of at the end. Ten of this module\'s hours happen in the week your application date arrives, so the first real interview is not the first rehearsal.'],
...
var AI_CHECKPOINTS = {
  ...
  M9:['One unit test that fails for the right reason, then passes',
      ...
      'Green required check in CI, plus a mutation score at or above 70%'],
