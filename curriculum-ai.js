...
{ id:'M21', layer:3, hours:15, dependsOn:['M5','M7'], kind:'EVIDENCE',
  title:'System Design and the Design Doc',
  artifact:'A one-page design doc with two rejected options for a bounded AI system, REVIEWED AND PUSHED BACK ON by a real reader before any code exists. Keep both the proposed and the built version; the delta is the interview material. PLUS three timed 45-minute design reps, 1h each, one per month from the application date, each on a different bounded system and at least one deliberately OUTSIDE your stack — you will walk in with real p99 numbers, a real durable queue and a real cost-per-completed-task table, and no reps at saying any of it.',
...
{ id:'M13', layer:4, hours:46, dependsOn:['M12'], kind:'EVIDENCE',
...
{ id:'M24', layer:6, hours:23, dependsOn:['M3','M19'], kind:'EVIDENCE',
...
{ id:'M27', layer:7, hours:12, dependsOn:['M16','M18'], kind:'EVIDENCE',
...
{ id:'M28', layer:7, hours:38, dependsOn:['M12','M26'], kind:'EVIDENCE',
...
var AI_TRACKS = [
  { id:'T1', title:'Job search \u2014 three channels, not one', hours:200, cadence:'3-4 h/week from the application date',
    rule:'Cold applications convert at 2-5% for a no-degree candidate; referrals convert at 28-40%. Same 200h, split: '
       + '(A) WARM/REFERRED ~70h \u2014 the named-humans list, and asking. (B) DIRECT-TO-HUMAN ~50h \u2014 email the '
       + 'engineer or founder who owns the problem, leading with one number from your own eval set, ~15/month. '
       + '(C) COLD ~80h at 6-7/month, treated as the control arm. Weight toward A in months 5-12 (a human vouching '
       + 'compensates for a half-built portfolio), toward B from month 13 when your measured numbers carry the email. '
       + 'Cap total researched applications at 130. Cold volume goes DOWN, not up. First two months, deliberately '
       + 'target companies you do not want. The modal response is silence; the success metric is "I learned what the '
       + 'funnel requires."' },
  { id:'T2', title:'Open-source PRs \u2014 and the ask', hours:80, cadence:'~8h/month, submission half starts after M7',
    rule:'Substantive = touches behavior, 20-200 lines, includes a test, survived a round of maintainer review. Three '
       ...
       + 'completely fine answer." Three asks by month 12. The same three people are your references AND your '
       + 'referral channel, and the ask comes first, while the work is fresh. This is the highest-conversion asset '
       + 'you build and it is worthless unconverted.' },
  { id:'T3', title:'The dependency spiral', hours:0, cadence:'structural',
    rule:'Every capstone imports an earlier capstone. Zero hours. But it is NOT the whole answer to decay: only 10 of '
       + '31 modules export anything downstream, and M1 \u2014 the largest and first \u2014 exports only the estimate '
       + 'log. After M1 closes around month 3 nothing touches it again. That is what the Sweep is for.' },
  { id:'T4', title:'Weekly cold re-build', hours:60, cadence:'45 min weekly',
    rule:'Pick an artifact from 3-8 weeks ago, rebuild its core from an empty file, original repo closed. Score 0-3. '
       + 'Re-open trigger now runs off SWEEP scores, which accumulate ~14 per module instead of 1.29, so rotation is '
       + 'error-driven rather than arbitrary. This slot is the bad-week minimum and is never double-booked \u2014 the '
       + 'narrated problems moved to T9 precisely because they were silently eating it.' },
  { id:'T5', title:'Reviewer relationships', hours:15, cadence:'~1 h/month, batched',
    rule:'Three roles, three deadlines: code reviewer (month 2), technical operator who can break a system cold '
       + '(month 4), mock interviewer (month 4 \u2014 NOT month 10; M25 needs a recruiter roleplay at month 4 and M28 '
       + 'forward-loads a mock to month 5). Agent fallback is one run per attempt, transcript kept including '
       + 'failures, and a failed agent review is logged as a failed gate.' },
  ...
  { id:'T7', title:'Writing \u2014 and shipping it somewhere', hours:30, cadence:'one piece per layer',
    rule:'Five pieces, each about something you MEASURED. Every piece is posted to at least one named place '
       + 'practitioners actually read, and everyone who responds goes on the named-humans list. Writing nobody reads '
       + 'is a diary; the distribution step is 15 minutes and it is the difference.' },
  { id:'T8', title:'Monthly re-plan \u2014 with a funnel table', hours:34, cadence:'2 h/month, survives into Employed Mode',
    rule:'Inputs that actually arrive: skill counts across the 20-40 postings screened this month (the real '
       + 'Python-and-cloud trip-wire signal, needing no employer to reply), your DELTA.md files, and a FUNNEL TABLE '
       + 'by channel \u2014 sent, replies, screens, loops, offers, contacts-per-hour. Two rules: a channel with zero '
       + 'human contacts for three consecutive months loses half its hours to the best channel; under 6 total human '
       + 'contacts by month 12 means the defect is the channel mix or the evidence, not the effort \u2014 and the '
  ...
  { id:'T9', title:'Narrated problems \u2014 the coding-screen instrument', hours:25, cadence:'~30 min/week from the application date',
    rule:'Budgeted and rubric-scored rather than counted: ~45 problems indexed to the EIGHT patterns '
       ...
       + 'reranker, sliding window for M3\u2019s rate limiting. One first pass ~30 min narrated with autocomplete '
       + 'OFF, one spaced re-solve at 2-4 weeks. Scored 0-3; three consecutive sub-2 in a pattern re-opens that '
       + 'pattern. Not 250 problems \u2014 you are not targeting loops that ask for them.' },
  { id:'T10', title:'The Sweep \u2014 retrieval practice', hours:15, cadence:'15 min weekly from month 4',
    rule:'Five cue-prompts, blank box, written, no notes, NO multiple choice. The item bank already exists and cost '
       + 'nothing to author: 223 concept strings and 152 failure-mode strings in the module data, one cue each. '
       + 'Score as a FRACTION \u2014 items produced over items in that module\u2019s array \u2014 so you cannot move '
       + 'the goalposts after seeing your answer. Selection is error-driven: weighted toward the lowest last score '
       + 'and toward the 21 modules that export nothing downstream and would otherwise never be touched again. This '
...
var AI_CUT_ORDER = ['M24','M23 second half','M14','M20 is already cut to 25h'];
// M30 removed from the cut order: it is the only module that owns seamlessness into
// real work, which is half the stated goal.      // ~80h back, in this order
...
M1:[
 ...
 ['TypeScript','JavaScript with the types written down next to the code. A checker reads them before the program runs and complains when a value could be the wrong kind. Used from here on.'],
 ...
 ['LLM','Large language model: the kind of model from M0B. LLM JSON is JSON the model wrote, which can be wrong in any way at all.'],
 ['Trust boundary','The line in your code where data from outside comes in: from a user, a model, another service. Everything past the line is trusted, so the checking happens at the line. This module owns the idea; M19 builds on it.'],
 ...
 ['Estimate log','A running record of how long you guessed a task would take, next to how long it took. Starts here and runs to month 12.'],
 ...
 ['Blind queue','The twenty unread bug-fix commits saved in M0A. An unseen bug from it is the test material, because a bug you have not seen cannot be recalled from memory.'],
...
M2:[
 ...
 ['Postgres','The database used for the whole program, and the local database you installed in M0A. Free, and the same one Supabase runs (M4).'],
 ['Query plan','The database\'s written step-by-step account of how it will answer a question. Read in M4.'],
 ...
 ['Idempotency','An operation is idempotent when doing it twice leaves things the same as doing it once. Named here; M3 owns it.'],
 ...
 ['Transaction, isolation','A transaction groups database operations so they succeed or fail together. Isolation is a separate question: how much two transactions running at once can see of each other. Atomic does not mean isolated. Both go deeper in M4.']],
M4:[
 ...
 ['Expand/contract','Changing a table\'s shape in two safe steps: add the new column first and move to it (expand), remove the old one later (contract). Nothing breaks in between. Run for real in M22.'],
...
M9:[
 ...
 ['Prompt caching, cache hit','The service remembers the start of a prompt you sent recently and charges a fraction to read it again. A hit is when that happened, visible in the usage object. Same idea as the M4 cache, on the provider\'s side.'],
 ...
 ['stop_details, discriminator','stop_details is the field that says more about the stop. A discriminator is the one field you branch on to tell cases apart, the tag from M1\'s discriminated unions; here it is the only reliable way to detect a refusal.'],
 ['Structured output, schema','Asking the model to reply in a JSON shape you declared, rather than free text. The schema is the written description of that shape (the same word as M2\'s database schema); a malformed generation is a reply that does not fit it.'],
 ...
 ['400, dead pattern','400 is the reply code an API sends when the request itself was wrong; M3 covers the codes. A dead pattern is a request shape that used to work and no longer does, so you keep every 400 you hit on one page.'],
 ...
 ['Tool-result message, prompt injection','A tool-result message carries the output of something the model asked your code to run (M17). Prompt injection is instructions hidden inside data the model reads, such as a web page or a user message, written to be obeyed. Contained in M17 and M19, never fixed.'],
...
M3:[
 ['HTTP','The rules browsers and servers use to talk: one request goes out, one response comes back, each with a status line, headers and a body. M0B ran on it without looking; here you read it byte by byte.'],
 ...
 ['Wire format, transport','The wire format is the exact shape of bytes you send, which the other end must match; the M20 client will consume this one. A transport is the piece of code that moves messages between two ends.'],
...
M5:[
 ...
 ['At-least-once delivery','Every job gets done at least once, and sometimes twice, because a worker can die after doing the work and before marking it done. The consumer must therefore be idempotent, reusing M3\'s dedupe table.'],
 ...
 ['Ingestion, backfill, re-embedding, agent state','Later modules\' work that rides this queue: document ingestion (M10), a backfill over existing rows (M12), re-embedding a corpus (M16), an agent\'s saved state (M17), webhook handling (M18). Each is defined in its own module.'],
...
M6:[
 ...
 ['Test runner, vitest','The program that finds tests, runs them and prints results. M1 used Node\'s built-in one; vitest replaces it here because mocks and fixtures come built in.'],
 ...
 ['Tool-use block','A part of a model reply asking your code to run a tool. Recorded here, used in M17.'],
 ['Multibyte character','A character stored as several bytes (M2). The fixture splits a frame in the middle of one.'],
...
M7:[
 ['Observability','Being able to tell what your running system is doing from what it emits, without adding code after the fact. This module owns it; M11 revisits it for evals.'],
...
M10:[
 ['Corpus','The whole collection of documents your app can search. M16 builds retrieval over the corpus you make here.'],
 ...
 ['Orphan-cleanup job','A job is one unit of work waiting in the M5 queue. This one runs on a schedule and deletes stored files that no database row refers to any more, so abandoned uploads do not pile up.'],
 ['Retention policy','A written rule for how long each kind of data is kept and when it is deleted. Stated now because M19 asks you to prove you follow it.'],
 ['Resumable','Able to be stopped part-way, by a crash or a kill, and continued later without starting over or repeating finished work. The M5 queue is what makes this possible.'],
 ...
 ['Decision log','A dated file where you write down each choice that would be expensive to reverse, and why you made it. It starts here, with the embedding model, and M27 publishes it.'],
...
M11:[
 ...
 ['Smoke set','A dozen or so cases chosen to catch obvious breakage quickly. Named after checking whether a machine smokes when switched on.'],
 ['Nightly','A run scheduled once a day, usually overnight, for work too slow or costly to do on every push.'],
 ['Recorded-fixture mode','Real model replies saved to disk once and replayed on later runs, using M6\'s record-replay client. A replayed run costs nothing and gives the same answer every time.'],
 ...
 ['Trajectory grader','A grader that scores the sequence of steps an agent took, not only its final answer. An agent is a model run in a loop that chooses its own next action; M17 builds one and uses these graders.'],
...
M21:[
 ...
 ['Cost per completed task','Total money spent divided by the number of tasks that actually finished, so retries and failed attempts count against you. M18 owns cost; here you bring the table into the room.'],
 ...
 ['Rollout','The plan for turning a change on for users in stages, so a problem is seen by a few before it reaches everyone. M22 makes you do it for real.'],
 ['Mock interviewer','A person playing the interviewer for practice, asking follow-ups you did not prepare for. Track 5 has you find one by month four.'],
...
M14:[
 ['Coding agent','An AI tool that goes beyond suggesting code: it edits files, runs commands and tests on its own, and keeps going until the task is done or it gives up. It is the coding form of the agent named in M11, a model run in a loop choosing its own next action, which M17 builds from scratch.'],
...
M15:[
 ...
 ['Estimate, estimate log','An estimate is your stated guess, before starting, of how long a task will take. The log records each estimate next to the actual time, and has been running since M1; its value is in the pattern, not in any one entry.'],
...
M16:[
 ['Retrieval','Finding the few passages in your corpus, the document store you built in M10, that are relevant to a question, so the model answers from them instead of from memory.'],
 ...
 ['Citation, span-level verification','A citation points from a sentence in the answer to the source passage that supports it. Span-level verification checks that the exact characters cited, by M10 offsets, really contain the claim.'],
 ...
 ['Re-embed','Producing new embeddings for every chunk, needed whenever you change the embedding model. It is a long job over every row, so it runs as a resumable backfill on the M5 queue.'],
...
M17:[
 ...
 ['Durable state','The agent’s progress saved to the database after every step, so a crash mid-run is resumed from the last step rather than started over. It lives on the M5 queue.'],
...
M19:[
 ...
 ['Guessed or replayed URL','Guessed: changing a number in a file address to reach someone else’s file. Replayed: reusing a signed link after it should have expired. Both are tried against your M10 bucket.'],
 ['OAuth, callback','OAuth is the standard way a user lets your app act on their account at another service, such as Google or GitHub, without giving you their password. The callback is the route on your site the other service sends the user back to when they say yes. Built in M24.'],
 ['Fetch tool','The M17 agent tool that makes web requests. It is a way into your system for anything on the web, so one of the five exploits goes through it.'],
 ...
 ['Client component','In the frontend framework used from M20, a component that runs in the browser. Any file it imports ships to the browser too, secrets included.'],
...
M20:[
 ...
 ['Protocol','The agreed shape of what goes over the wire between two programs. Yours is the SSE frame format you defined in M3; the client has to speak it exactly.'],
 ...
 ['Prebuilt default','The transport that ships with the popular AI frontend library and expects that library’s own wire format. You replace it so your client speaks M3’s format.'],
...
M8:[
 ...
 ['pydantic','The Python library that checks incoming data against a declared shape at runtime and rejects what does not fit. It plays the role Zod plays in M1.'],
 ...
 ['Track 2','The parallel track of this program in which you send pull requests to open-source projects. Routed means the PR for this gate goes to a Python project chosen from that track.'],
...
M23:[
 ['Port, ported','Rewriting a program in another language so it does the same thing. Porting the M11 runner means the Python version must produce the same scores.'],
 ...
 ['Backend, Node-primary','The backend is the server side of an app. A Node-primary backend is one whose main server language is JavaScript; the trigger moves this module earlier if most postings want something else.'],
...
M24:[
 ...
 ['Access token','The short-lived credential each request to the provider carries. When it expires, the refresh token from M22 fetches a new one; automatic refresh means the app does this itself, on the provider’s schedule.'],
 ...
 ['State parameter','A random value your server creates when sending the user out and checks when they come back. Without it an attacker can complete the callback on the user’s behalf; that is the CSRF from M19.'],
...
M25:[
 ...
 ['Freelance, freelance client','Freelance work is paid work done as a contractor for someone who is not your employer. The client here is the person or company that paid for it, not the browser side of an app as in M0B; a real one can vouch for you as a reference.'],
 ...
 ['Delta (of two numbers)','The difference between two numbers. Here it is the money you keep under one arrangement minus the money you keep under another, computed by you rather than taken from the headline. M27 uses the same word for the difference between two scores.'],
...
M26:[
 ...
 ['Product-spec form','A README written like the M0B specification: who the app is for, what it does, what it costs, how good it is, what it refuses to do. Not a list of what is inside it.'],
...
M27:[
 ['Layer','The program\'s grouping of modules into stages, numbered 0 to 8. Layer 5 is the AI modules M16 to M19, and this module waits for it because that is where the measured numbers come from.'],
 ...
 ['Protocol (procedure)','A fixed procedure followed the same way each time so results can be compared. The three-stranger protocol is M26\'s gate: three people, five minutes each, answers in writing first. Not the wire protocol of M20.'],
...
M28:[
 ...
 ['Foreign repo','A repo you did not write and have never opened. The bug fixes here come from the M0A blind queue, so every one is foreign.'],
 ...
 ['Track 4, weekly slot','The parallel tracks are the work that runs alongside the modules every week. Track 4 is the weekly 45-minute rebuild of something you made a few weeks ago; its slot is the fixed time in your week it occupies.'],
 ...
 ['System design round','The round where you design a system at a whiteboard for forty-five minutes and defend it, rehearsed in M21. Mid-level means the bar is a bounded system with its failure path, not a global one.'],
 ...
 ['Cold (question)','With no warning and no preparation, as a question asked for the first time in the room; not the cold cache of M4. The rehearsal is so that cold still has a story ready.'],
...
M29:[
 ...
 ['Component (of a codebase)','One self-contained part of a larger codebase, such as the billing code or the search code. Not the React sense from M20.'],
 ...
 ['Monthly re-plan','The two hours each month, from Track 8, where you re-read the plan against what actually arrived: postings screened, the funnel table, your DELTA notes. It continues after hire with the job as a new input.'],
...
M30:[
 ...
 ['Review latency','How long a PR waits between being opened and getting its first review. Your own data comes from the Track 2 PRs you sent to open-source projects, and it decides how many PRs to keep open at once.'],
