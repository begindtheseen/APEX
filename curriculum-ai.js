...
var AI_LAYERS = [
  ...
  { id: 8, name: 'Employed Mode',         purpose: 'Getting hired before you finish is the normal good outcome. This layer is what changes on day one of the job.' }
];
...
{ id:'M5', layer:1, hours:61, dependsOn:['M4'], kind:'LAB',
  ...
  artifact:'pg-lab against a local Supabase stack. OPEN WITH A 3-HOUR WIN: 50k rows, one slow query, one EXPLAIN, one index, one measured 200x speedup. Then the 5,000,000-row rig: twelve hand-written queries with plans before and after, a slow-to-fast loop with wall-clock numbers from a load generator you wrote, an RLS policy set benchmarked correct-but-slow vs fast, and the legacy-key rotation as an artifact. Plus asymptotic complexity, taught here because here it is measurable. Then the flagship moves onto hosted Supabase: sign-in, one table scoped to the signed-in user, one row-level security policy, and a pooled connection, so the app now has accounts and keeps data between visits.',
  exports:['load generator -> M23'],
  gate:{ referee:'Your reviewer, holding an EXPLAIN plan you have never seen, taken from their own work or from a public slow-query log.',
...
{ id:'M16', layer:4, hours:28, dependsOn:['M14','M15'], kind:'EVIDENCE',
  ...
  artifact:'A defect log from handing an agent three real tickets in a repo you did not write, naming for each defect WHICH earlier module let you catch it. A one-page delegation policy, enforced by a pre-commit hook that has actually blocked an agent-authored change twice on real work. A spec-driven slice with a log of every place the spec was underspecified and what the agent did in the gap. AND a throughput artifact: one non-trivial flagship feature shipped in a timebox using agents, transcript preserved, measured against a similar feature written by hand (hours, review findings, defects reaching production).',
...
{ id:'M23', layer:6, hours:48, dependsOn:['M5','M9','M12'], kind:'EVIDENCE',
  ...
  artifact:'The flagship full pipeline: OIDC secrets with no stored keys (the model API key is the one DOCUMENTED exception, scoped and spend-capped; a second exception is added if M26 is already built), required checks including the M12 eval gate, a feature-flagged release, a rollback rehearsed under a timer measured from the DASHBOARD not a stopwatch, an expand/contract migration run through the pipeline while the M5 load generator fires with zero failed requests, one burn-rate alert that fired for a real reason. Plus one dockerized cloud deploy with an IAM role you wrote and can explain.',
...
{ id:'M24', layer:6, hours:15, dependsOn:['M9'], kind:'LAB',
  ...
  artifact:'Current tooling (uv, ruff, one pinned type checker — stated shelf life). Python semantics where they differ from TypeScript. pytest. Type hints with a checker in CI. pydantic as the validation boundary. One typed FastAPI route with a test.',
...
{ id:'M30', layer:7, hours:38, dependsOn:['M28'], kind:'EVIDENCE',
  ...
  artifact:'A recorded 10-minute flagship walkthrough in decision language. Three timed foreign-repo bug fixes from the bug-fix commits saved unread in M1 with assistant transcripts annotated. Two recorded mock defenses with a real person who pushes back. Plus about 8 hours of behavioral rehearsal against the M0 incident log — twenty real stories accumulated at zero marginal cost since month one. The narrated-problem log lives in Track 9\u2019s weekly slot (Tracks tab).',
  gate:{ referee:'Three referees: your mock interviewer for the first two, your reviewer for the third.',
         pass:'(1) Solve an unseen problem out loud in about 30 minutes with autocomplete disabled. (2) Drive an assistant through an unfamiliar bug UNDER OBSERVATION, narrating every point where you VERIFIED rather than accepted. (3) Answer three behavioral questions cold with three different stories, none about a decision made alone.',
...
var AI_TRACKS = [
  ...
  { id:'T5', title:'Reviewer relationships', hours:15, cadence:'about an hour a month, in one sitting',
    rule:'Three people, three deadlines. A code reviewer before M1\u2019s gate. Someone who runs systems for a living and can try to break yours, by M10. A mock interviewer before M13, and in any case before your application date, because M13 and M30 both gate on one. If no person is available for a gate, an AI assistant may stand in once per attempt; the whole conversation is kept, failures included, and a failed AI review counts as a failed gate.' },
  ...
  { id:'T7', title:'Writing — and posting it somewhere', hours:30, cadence:'one piece per layer, Layers 2 to 6',
    rule:'Five pieces, each about something you measured. Every piece is posted to at least one named place where working engineers actually read it, and everyone who responds goes on your list of real people. Writing nobody reads is a diary; posting it takes 15 minutes and is the whole difference.' },
...
M1:{concepts:['What a program is: a file of instructions a runtime (here, Node) reads top to bottom','The terminal as a place you type commands, and what a working directory means','Variables, functions, arguments and return values \u2014 the four things every program is made of','Conditionals and loops, and why a loop that never ends is the first bug everyone writes','Arrays and objects: a list of things, and a thing with named parts','Reading an error message: the type, the message, the file and the line number','What a test is \u2014 code that runs your code and says yes or no without you looking','git as a save-point system: what a commit is, and what branch, remote and getting back mean'],
 mistakes:['Following a tutorial to a working app and mistaking that for being able to write one. The gate here is an empty directory for exactly this reason.','Reading the error message as noise instead of as the answer. It names the file and the line; nine times in ten it also names the mistake.','Copying code you cannot explain, which converts a 5-minute bug into a 2-hour one because you cannot form a hypothesis about your own program.','Setting up a perfect environment (your editor and installed tools) instead of writing programs. A slightly wrong editor that you use beats a perfect one you configure for a week.','Not committing until it works. The point of a save-point is to have one from before it broke.']},
...
M24:{concepts:['Environments and dependencies on current tooling (uv, ruff, one pinned type checker)','Python semantics where they differ from TypeScript: names vs values, LEGB, truthiness, generators','pytest','Type hints with a checker in CI','pydantic as the runtime validation boundary','One typed FastAPI route'],
 mistakes:['Writing TypeScript with Python syntax: classes everywhere, raw dicts, no type hints, camelCase. Reviewers read that instantly.','Reaching for conda/poetry/pyenv because a pre-2024 tutorial said to.','Assuming type hints behave like TypeScript’s compile-then-trust contract.']},
M6:{concepts:['Tokens as the unit of everything','The messages array, roles, statelessness, what a system prompt mechanically is','Why text in a user or tool-result message can be forged — that is prompt injection','The context window: what fills it, what degrades before it fills','max_tokens and the full stop_reason enum','Pricing: input/output asymmetry, caching as a byte-exact prefix match','Adaptive thinking and effort — the first quality-trading lever after caching','Discovering capabilities from the Models API rather than a hard-coded table','The structural boundaries — what no prompt fixes','Prefill vs decode and the KV cache — the one mechanism explaining three facts this module asserts: why caching needs a byte-exact PREFIX, why conversation cost grows quadratically, and why time-to-first-token and inter-token latency are different numbers. No training, no backprop, no attention math. Then measure it: probe a long prompt and a short one and show TTFT scaling with input while inter-token latency does not'],
 mistakes:['Using another provider’s tokenizer (the provider is the company running the model service) or a chars/4 rule to budget tokens.','Treating max_tokens as a cost cap. It is a ceiling the model is unaware of, so it truncates mid-thought.','Checking for an empty content array to detect a refusal. The array is populated; stop_details is the discriminator, and that branch never fires.','Interpolating anything dynamic near the front of the system prompt. One changed byte invalidates the cache and the failure is completely silent.','Optimizing input tokens while ignoring that output costs several times more.','Asking the model to rate its own confidence and making decisions (routing) on that number. It looks like a probability and behaves like a vibe.']},
...
M12:{concepts:['Trace capture: the exact input, usage, stop_reason and attempt count','Error analysis: open coding to axial coding, producing a named taxonomy with counts','What an eval is — a dataset plus a runner — and why it is not a test','Assertion graders before any model grades anything','LLM-as-judge calibrated against human labels, reported as TPR/TNR','Inter-annotator agreement and rubric revision','Train/test discipline: split at creation, open test once','Structured output as a reliability mechanism, and its limit','CI gate economics: tiered gates, recorded fixtures, a dollar ceiling','Prompt and model lifecycle: versioning, pinned IDs, the forced migration'],
 mistakes:['Reporting accuracy instead of TPR/TNR. When failures are rare, a judge that always says pass scores 92% and is worthless. This invalidates more eval work than anything else.','Generic categories — "hallucination", "unhelpful". Unactionable. "Calendar Scheduling Failure" is a fix; "poor coherence" is a shrug.','Delegating the labeling to an LLM. It clusters notes you already wrote. People discover what they care about through labeling.','Building the eval set from cases you invented. An imagined set measures imagination.','A runner that re-implements the model call to keep the eval clean. It then measures a different system than the one that ships.','Using the same model as generator and judge.','Believing schema enforcement solved reliability. It solves shape, not content.']},
...
M28:{concepts:['A resume that maps each claim to a repo','Two pinned repos now; the pinned three (flagship, open-source history, a write-up) arrive in M29','Lab vs Evidence — labs are private','Never naming the curriculum'],
 mistakes:['Pinning the lab repos, which reads as coursework.','Naming the curriculum. The correct sentence is "I spent the last year building and operating X."','A skills list of 22 technologies. Six you can be grilled on read as competence. Do not put "Prompt Engineering" on it.']},
M29:{concepts:['The README as product spec plus decision record','Publishing the failing v1 numbers alongside the improved ones','The write-up genre that converts: a numbered account of something that went wrong in your own system'],
 mistakes:['The README as a feature list. The stack list is the least interesting thing in the repo and it is what 90% of portfolios lead with.','Hiding the v1 numbers because they were bad. The improvement delta IS the evidence; a single good number could have been luck.','Tutorials that duplicate a thousand existing posts.']},
...
M31:{concepts:['A second operating contract at an employed weekly budget (5-8h, not 18)','Retargeting the cold rebuild to the employer’s codebase','Re-contracting or replacing the reviewer before the start date','Manager checkpoints at week 6 and week 14, scripted in advance'],
 mistakes:['Treating hire-before-completion as failure. The arithmetic makes it the normal good outcome.','Letting every feedback instrument terminate on hire day.','Asking "am I where you expected?" in week one, when a manager’s answer is most generic.']},
M32:{concepts:['The 30/60/90 plan and the week-one manager questions','The asking-for-help format: what you tried, expected, saw, and currently believe','Org mapping from git history','Inheriting an AI system you did not build, with a done-state: a one-page note saying where the prompts live, which are load-bearing, what the implied eval was, what the author was evidently afraid of (inferred from the defensive instructions in the prompt text), and the single measurement you would run first','A WIP policy computed from your own review-latency data'],
 mistakes:['Silent struggle. Self-taught plus remote is the highest-risk combination, and being stuck two days on a 5-minute unblock is how new hires are let go at month three.','Working one thing at a time, so four months of six-day PRs read on a cycle-time dashboard as slow delivery.','Asking for the answer instead of stating your current best hypothesis.']}
};
...
M0:[
 ...
 ['Ceiling, scope','The program assumes you can give at most 18 hours a week for a long time; that is the ceiling. Scope is how much fits under it in your runway: all 33 modules, or the Spine. When it does not fit, the scope is cut, never the ceiling. Month numbers anywhere in this program assume 18 hours a week; The Plan shows your own.'],
 ['Layer','The program’s grouping of modules into nine stages, numbered 0 to 8, shown on the Home tab. Each layer opens when the one before it is done.'],
...
M3:[
 ...
 ['Third pass condition','A third thing the gate requires, on top of the two in its pass line. It is written separately because it is checked on a bug you have not seen.'],
...
M5:[
 ...
 ['Key rotation','Replacing a secret with a fresh one and retiring the old. A legacy key is an older one on its way out; Supabase is retiring its older long-lived keys, and doing that swap is one of this module\'s artifacts.'],
...
M6:[
 ...
 ['Prompt caching, cache hit','The service remembers the start of a prompt you sent recently and charges a fraction to read it again. A hit is when that happened, visible in the usage object. Same idea as a cache in front of a database, on the provider\'s side (the provider is the company running the model service).'],
...
M8:[
 ...
 ['Back in the pool','Ready to be picked up again. A job whose lease expired goes back in the pool, so the next free worker claims it.']],
M9:[
...
M17:[
 ...
 ['CSV','Comma-separated values: a plain text table, one row per line with commas between the columns. Spreadsheets open and save it, which is why exports and imports use it.'],
 ['Nullable','A column allowed to hold no value (null). A new column is added nullable so existing rows stay valid until it is filled.'],
 ['Dual write','Writing the same data to the old column and the new one while the migration is in progress, so both stay correct until the contract step drops the old one.']],
M18:[
...
M19:[
 ...
 ['Blocked host, metadata endpoint','A blocked host is an address you have decided tools must never reach, used to prove the allowlist works. 169.254.169.254 is the metadata endpoint (an endpoint is one address a server answers at): the address inside a rented server where the machine’s own credentials are handed out; a tool that can fetch it can steal them.'],
...
M22:[
 ...
 ['Server/client boundary','The line between code that runs on the server and code that runs in the browser. In Next.js, the framework used here, components are server-side until marked otherwise.']],
M23:[
 ...
 ['CSV','Comma-separated values: a plain text table, one row per line with commas between the columns. The figure’s export lands in one.'],
 ['Nullable','A column allowed to hold no value (null). The expand step adds the new column nullable so existing rows stay valid until it is filled.'],
 ['Dual write','Writing the same data to the old column and the new one while the migration is in progress, so both stay correct until the contract step drops the old one.']],
M24:[
...
M26:[
 ...
 ['Session','The server’s record that a user is logged in, tied to a cookie the browser sends back on every visit. The connection you build is stored against the session’s user.'],
 ['code_verifier','The PKCE secret your server makes and keeps for one login. The scrambled version it sends out is the code_challenge; the verifier goes back when the code is exchanged, proving the same server started and finished.'],
...
M27:[
 ...
 ['Self-employment tax, SE tax','The extra tax a contractor pays that an employer would otherwise cover (the employer’s half of FICA), about an added 7.65 percent of income in the US. It is the first reason a 1099 headline number is worth less than it looks.'],
...
M28:[
 ...
 ['Channel, named channel','A place where you can reach people who are not your friends: a Discord server (a chat community), a subreddit (one topic’s forum on Reddit), a mailing list, a meetup. Named means you write down which one each stranger came from, so the same channel is not reused for the next three.'],
...
M29:[
 ...
 ['Protocol (procedure)','A fixed procedure followed the same way each time so results can be compared. The three-stranger protocol is M28\'s gate: three people, 5 minutes each, answers in writing first. Not the wire protocol of M22.'],
...
var AI_CHECKPOINTS = {
  ...
  M1:['A terminal you opened yourself, node --version answering, and an empty file you created from the command line',
      ...
      'The 30-minute install check done, your first unseen task scored as a fraction, and the twenty commits saved unread'],
  ...
  M9:['One unit test that fails for the right reason, then passes',
      ...
      'Green required check in CI, plus a mutation score of 70% or more'],
  ...
  M18:['The measuring instrument first: a golden set, split dev/test at creation',
      ...
      'Contextual retrieval measured against the same baseline',
